...
**Nao se escreve HTML.** So `.cronus` no repo. HTML e por tras: o kernel emite na saida do `cronus run`. Arquivo `.html` / `.css` / `.tsx` neste repo e erro. `style_block` e `template "<div"` sao proibidos.
