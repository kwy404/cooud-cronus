...
be documented in an SDD first;

have parser/AST tests;

have renderer/runtime tests;

have invalid-syntax tests;

be added to LANGUAGE.md only after implementation is real;

avoid ambiguous grammar and preserve backward compatibility unless a migration is explicitly approved.
...
inspect source implementation;

inspect tests/docs/examples;

enumerate props/variants;

enumerate visible states;

enumerate keyboard behavior;

enumerate accessibility behavior;

enumerate theme/token dependencies;

enumerate responsive behavior;

enumerate animation behavior;
...
write/update the SDD;

add failing tests first or in the same atomic change;

implement the smallest reusable primitive;

run focused tests;

run full cargo test;

test old demos/templates for regressions;

add a new .cronus example demonstrating the capability;

update LANGUAGE.md to REAL only when verified;
...
Every meaningful cronus-ui source capability has a parity-matrix row.

Every row is VERIFIED or explicitly approved as INTENTIONAL_DIFFERENCE.

All migrated examples are authored in native .cronus.

No React/Next/Bun/npm/node_modules dependency is required for normal target runtime.

Semantic theme/tokens are native and tested.

All component waves are functionally implemented.

Interactive components pass behavior + keyboard tests.

Accessibility gates pass.

Light/dark/theme override behavior passes.

RTL/localization contract passes for supported components.

Source-vs-target visual evidence exists for canonical states.

Native CLI/scaffolding/compose workflows replace the source developer intent.

Docs/showcase use executable .cronus examples.

Full kernel test suite passes.

Existing kernel demos/templates do not regress.

Harness checks pass in one documented command.

LANGUAGE.md accurately reflects implementation.

migration/FINAL-REPORT.md exists with metrics, known intentional differences, evidence, and migration summary.

Another clean environment can clone the target, run the documented setup, and reproduce the verification.

No unreviewed compatibility hacks remain.
...
target capability/gap inventory;

proposed CRONUS-native component model;
...
first small vertical slice to implement;

exact acceptance criteria for that slice.
